    ...
    def compute_grid_pixel_sum_lines(self):
        ...
        def save_txt():
            if not self.img_path:
                messagebox.showinfo("无法保存", "当前图片无有效文件路径，无法保存。")
                return
            try:
                out_path = os.path.splitext(self.img_path)[0] + ".txt"
        ...
        Button(btn_frm, text="复制每行一条到剪贴板", command=copy_lines).pack(side=tk.LEFT, padx=4)
        Button(btn_frm, text="保存为同名TXT文件", command=save_txt).pack(side=tk.LEFT, padx=4)
