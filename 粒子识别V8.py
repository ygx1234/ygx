...
class ImagePanel(Frame):
    ...
    def _create_widgets(self):
        ...
        # 修复乱码：保存标注图
        Button(detect_frame, text="保存标注图", command=self.save_annotated_image).grid(row=0, column=9, padx=(8, 6))
    ...
    def compute_grid_pixel_sum_lines(self):
        ...
        【新增】在弹窗底部增加：
        - 保存为同名TXT（覆盖）：image.tif -> image.txt（与图片同目录）
        """
        if self.orig_pil is None:
            messagebox.showinfo("统计结果", "尚未加载图像或无显示内容。"); return
        if not self.show_grid_var.get():
        ...
        # ===========
        # 【关键改动】三段布局，确保底部按钮始终可见
        # ===========
        top = tk.Toplevel(self)
        top.title("网格灰度和（每行一条，便于复制）")
        top.geometry("520x640")  # 增加高度，避免按钮被挤出可视区域

        header = Frame(top)
        header.pack(fill=tk.X, padx=8, pady=(8, 2))
        Label(header, text=f"图片：{self.img_name}", anchor='w').pack(fill=tk.X)
        Label(header, text=f"网格：{rows} 行 × {cols} 列，形状={self.grid_shape_var.get()}", anchor='w').pack(fill=tk.X)
        if stats_upper is not None:
            Label(header, text=f"统计忽略阈值：大于 {stats_upper} 的像素不计入总和", anchor='w', fg="#444").pack(fill=tk.X)

        body = Frame(top)
        body.pack(fill=tk.BOTH, expand=True, padx=8, pady=8)
        scrollbar = tk.Scrollbar(body)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        txt = tk.Text(body, wrap=tk.NONE, yscrollcommand=scrollbar.set, font=("Consolas", 12))
        txt.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar.config(command=txt.yview)
        ...
        footer = Frame(top)
        footer.pack(fill=tk.X, padx=8, pady=(0, 10))
        ...
        def save_same_name_txt_overwrite():
            # 必须有真实路径才能“同名保存”
            if not self.img_path or (not isinstance(self.img_path, str)) or (not os.path.isfile(self.img_path)):
                messagebox.showerror("保存失败", "当前图片没有有效文件路径（img_path），无法保存同名TXT。")
                return
            base, _ = os.path.splitext(self.img_path)
            out_path = base + ".txt"
            try:
                save_text_overwrite(out_path, "\n".join(lines) + "\n")
                messagebox.showinfo("保存成功", f"已保存到（覆盖同名）：\n{out_path}")
            except Exception as e:
                messagebox.showerror("保存失败", f"无法写入文件：{e}")

        Button(footer, text="复制每行一条到剪贴板", command=copy_lines).pack(side=tk.LEFT)
        Button(footer, text="保存为同名TXT（覆盖）", command=save_same_name_txt_overwrite).pack(side=tk.LEFT, padx=(10, 0))
